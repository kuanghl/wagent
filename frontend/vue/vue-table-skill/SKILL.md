...
description: Vue 通用表格技能。base-table / base-loading / base-paginated 共 3 个独立组件、23 种形态。Vue3 + TypeScript 泛型，零 HTML5 标签，全部用 <div> <span> + CSS3 实现。对齐 vue-theme-skill / base-card 容器契约，禁止混入任何第三方 UI 库。强依赖 base-button / base-status / base-card。支持固定列、操作列、排序、筛选、选择、展开、编辑、树形数据、虚拟滚动、拖拽排序等核心功能。触发词："做一个表格"、"数据表格"、"用户表格"、"产品列表"、"订单表格"、"可排序表格"、"可筛选表格"、"带分页表格"、"可选择行表格"、"固定列"、"操作列"、"可编辑表格"、"树形表格"、"加载中效果"、"分页组件"、"base-table"。
---

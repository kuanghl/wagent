...
description: Vue 通用状态/标签组件技能。base-status：Vue3 + TypeScript 泛型，支持 7 种 type（primary/success/warning/danger/info/default/neutral）、5 种 variant（solid/light/outline/ghost/dot）、3 种 size（sm/md/lg）、圆角/方形/可关闭/带图标。零第三方组件库，必须嵌入 base-card 容器。触发词："做一个状态标签"、"做一个徽章"、"chip"、"状态指示器"、"success 标签"、"danger 标签"、"info 标签"、"带点的标签"、"可关闭的标签"、"带图标的标签"、"base-status"。
---
